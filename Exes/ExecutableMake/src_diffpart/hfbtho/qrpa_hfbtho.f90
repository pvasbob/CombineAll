...
Module qrpa_hfbtho
  !---------------------------
  Use hfbtho_utilities, only : ipr, pr
  Use hfbtho_utilities, only : zero, half, one, two
  Use hfbtho, only : nghl, nuv, nqp, lout, lfile, n00, b0, npr, si, nb, id,  fl, fh, REqpN, REqpP  
  Use hfbtho, only : keyblo, blo123d, bloblo, txb  
  !---------------------------
  Implicit None
  ...
  !
  Complex(pr), Public, Allocatable         :: H20N_c(:),H20P_c(:),H02N_c(:),H02P_c(:)
  !
...
contains
  ...
  Subroutine qrpa_coulom
    ...
    Real(pr), Save :: zd2,y1,y2,xx1,s1,vik,f,r,r1,fac1,fac2,rr2,z,z1,zd1,t,  &
                      ...
                      beta,xxx
    !omplex(pr) :: cone, czero
    !one = 1.0_pr;  czero = 0.0_pr
    !
...
!stop
  ...
  Subroutine qrpaINI_CUT_ALLOCATE
    ...
    !
    Allocate(H20N_c(iQRPA_LEN),H20P_c(iQRPA_LEN),H02N_c(iQRPA_LEN),H02P_c(iQRPA_LEN)) 
    !
...
!end if
  ...
  !
  !=================================================================================================================================
  Subroutine qrpa_H20(V,U,H,D,BH,BD, H20, H02)
    !
    ...
    Complex(pr), Intent(out)  :: H20(iQRPA_LEN),H02(iQRPA_LEN)
...
!FF2=aux2
  ...
  Subroutine Calculate_QRPA_H20
   ...
   !if(ENABLE_P_EFA) Call qrpa_H11(VqpP,UqpP,hP_c,dP_c,BhP_c,BdP_c,h11P_c,Bh11P_c)
    ...
    !---------------------------------------------------------------------
    Call qrpa_H20(VqpN,UqpN,hN_c,dN_c,BhN_c,BdN_c, H20N_c, H02N_c)
    Call qrpa_H20(VqpP,UqpP,hP_c,dP_c,BhP_c,BdP_c, H20P_c, H02P_c)
    !
    !---------------------------------------------------------------------
    !---------------------------------------------------------------------
    ! add external fields
...
!    Complex(pr), Allocatable :: training_energy(:)
!    Integer :: qqi 
!    Integer :: qqj 
...
!    !===================================================================rbmend
    ...
    open(50, file='50_iteration', access='append') 
    !----------------------------------------------------------------------------------------------------
    ! QRPA INITIALIZATION
    !----------------------------------------------------------------------------------------------------
    If(iqrpa.Eq.0) Then
    ...
    Do iter_qrpa=0,max_iter_qrpa
       ...
       If(iter_qrpa.Lt.500.Or.si.Lt.qrpa_eps*10.0_pr) Then
          Do iw=lout,lfile
             ...
             write(50,'(i4,a,1x,f12.8,f5.2," | ",6(1x,g12.6)," | ",3(1x,g12.6)," | ",f7.2)') &
                  iter_qrpa,qrpa_bbroyden,si,qrpa_alphamix,&
                  ReStrengthN,ReStrengthP,ReStrengthN+ReStrengthP,ImStrengthN,ImStrengthP,ImStrengthN+ImStrengthP,&
                  ImTransStrN,ImTransStrP,ImTransStrN+ImTransStrP,time2     !krisa! new variables introduced, snz_qrpa(1:2) variables were removed
...
!         qqi  =1    ! do this to ensure qqi has value of 1.
!         !qrpa_omega = training_energy(qqi)
!          write(*,*) qrpa_omega
!         !Call qrpa_HFBTHO_solver
!          !
!          write(*,*) 'nuv: ', nuv
!          write(*,*) '2*nuv: ', 2*nuv
!          write(*,*) 'iQRPA_len: ', iQRPA_len
!          write(*,*) 'size(XN_c)', size(XN_c)
!          
!       !  training_energy = cmplx(qrpa_omega_points_re, qrpa_omega_points_im)
!       !  rbmdim = max_qrpa_points
!          rbmdim = 1
!          rbmdim2 = 4*rbmdim
!          write(*,*) 'before alloocate'
!          Allocate(XN_train_c(iqrpa_len,rbmdim2),   XP_train_c(iqrpa_len,rbmdim2),   YN_train_c(iqrpa_len,rbmdim2),   YP_train_c(iqrpa_len,rbmdim2), &
!             &   H20N_train_c(iqrpa_len,rbmdim2), H20P_train_c(iqrpa_len,rbmdim2), H02N_train_c(iqrpa_len,rbmdim2), H02P_train_c(iqrpa_len,rbmdim2))
!          ! initialization
...
!          !
!          write(*,*) 'after call qrpa_HFBTHOP_solver'
!          ! Save results
!          XN_train_c(:,qqi) = XN_c(:); YN_train_c(:,qqi) = YN_c(:)
!          XP_train_c(:,qqi) = XP_c(:); YP_train_c(:,qqi) = YP_c(:)
!
!          qrpa_omega_rec = qrpa_omega   ! record current qrpa_omega
!          qrpa_omega=0.0d0
!          Call qrpa_Green_function(qrpa_omega) ! calculate 1/(E_\mu + E_\nu)  
!
!          qrpa_omega = qrpa_omega_rec   ! record current qrpa_omega
!
!          H20N_train_c(:,qqi) = H20N_c(:); H02N_train_c(:,qqi) = H02N_c(:)
!          H20P_train_c(:,qqi) = H20P_c(:); H02P_train_c(:,qqi) = H02P_c(:)
!       write(*,*) "Inside qqi"
!       !nd Do
!
!
!   ! save training results to a file
!   !llocate(F20N_ext_c(nuv,9),F20P_ext_c(nuv,9),F02N_ext_c(nuv,9),F02P_ext_c(nuv,9))
!   Allocate(F20N_ext_c(iqrpa_len,1),F20P_ext_c(iqrpa_len,1),F02N_ext_c(iqrpa_len,1),F02P_ext_c(iqrpa_len,1))
!   F20N_ext_c=0.0_pr; F20P_ext_c=0.0_pr; F02N_ext_c=0.0_pr; F02P_ext_c=0.0_pr;
!   write(*,*) "F20_ext_allocated"
...
!   qqj = 1      
!   F20N_ext_c(:,qqj) = FhN_c(:); F20P_ext_c(:,qqj) = FhP_c(:); F02N_ext_c(:,qqj) = BFhN_c(:); F02P_ext_c(:,qqj) = BFhP_c(:)
!   !
!
!   write(*,*) "F20_ext_assigned"
!  !! restore
...
!
!   !pen(unit=trainingdata,file=fam_training_outputfile,action='write',form='unformatted')
!   !pen(unit=trainingdata,file=fam_training_outputfile,action='write')
!   write(*,*) "before write to output"
!   trainingdata = 20
!   Open(unit=trainingdata,file="emulatortraining.dat",action='write')
!   Write(trainingdata, *) iqrpa_len*2  ! dimension of the neuron and proton 2qp
...
!   !o i = 1, rbmdim
!   Do i = 1, 1
!      Write(trainingdata, *) qrpa_omega, XN_train_c(:,i), XP_train_c(:,i),YN_train_c(:,i), YP_train_c(:,i), &
  ...
  Subroutine write_amplitudes
   ...
   !End Do
    ...
    !--------------------------------
      ...
      Norm_P = 0
      !
     !imag_qrpa_omega = imagpart(qrpa_omega)
      imag_qrpa_omega = aimag(qrpa_omega)
     !real_qrpa_omega = realpart(qrpa_omega)
      ...
      write(46,*) real_qrpa_omega,',', Norm_N+Norm_P,',', ImTransStrN + ImTransStrP 
